...
- Guard every sourced file against double-inclusion: `declare -r __vendor_bashkit_lib_<name>_sourced="true"` at the top, checked before re-sourcing dependencies at the bottom (each file re-sources `bash-logger-adapter/adapter.sh` and `options.sh` itself — don't assume load order from a caller).
...
There is no test harness (no bats, no shellcheck config beyond the `# shellcheck shell=bash` / `# shellcheck disable=...` directive comments already in each file). Validate changes by sourcing the affected file from within a consumer repo (e.g. `titan`) that has the `bash-logger-adapter/adapter.sh` shim available, or by running `shellcheck` directly against the file.
